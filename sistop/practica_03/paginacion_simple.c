...
#define CAPACIDAD_MEMORIA 6   // en kilobytes
#define MARCO_PAGINA      512 // en bytes
#define MARCOS_SO         2
...
void tt_lista_crear(void) {
    for(int i = 0; i < NUM_TAREAS; i++) {
        if(TTCabeza == NULL) {
            ...
            TTCabeza->num_lineas_codigo = aleatorio_crear(99, 520);
...
void paginacion_simple(void) {
    ...
    for(int i = 0; i < NUM_TAREAS; i++) {
        ...
        for(int j = 0; j < num_marcos_mem; j++) {
            ...
            if(lin_codigo_tarea < (num_marcos_disp * LINEAS_CODIGO_PP)
                    && TMMAux->estado != 1) {
                printf("Asignando J%d en marco %d\n",
                        TTAux->num_tarea, TMMAux->num_marco);
                lin_codigo_tarea = lin_codigo_tarea - 100;
                TMMAux->estado = 1;
            }
            /* printf("En tarea J%d con marco %d\n", TTAux->num_tarea, */
            /*         TMMAux->num_marco); */
            TMMAux = TMMAux->siguiente;
        }
        TTAux = TTAux->siguiente;
